...
export default async function handler(request, response) {
  try {
    ...
    const presetScores = [1, 2, 3].map((presetNo) => {
      ...
      if (!stats || !items) return { presetNo, score, combatPower };
      ...
      const hyperStatEXP = hyperStatData?.hyper_stat_preset_1?.find(
    ...
    const combinedItemData = {
      item_equipment: Array.from(
        new Map(
          presetItems
            .flatMap((p) => p?.item_equipment || [])
