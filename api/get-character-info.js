...
export default async function handler(request, response) {
  try {
    ...
    // ✨ [핵심 수정] 상세 페이지에 보여줄 데이터는 현재 활성화된 프리셋(1번) 기준으로 조합하되,
    // 모든 아이템 정보를 합쳐서 'item_equipment'에 다시 넣어줍니다.
    ...
    const itemDataForDetail = presetItems[0] || {}; // 기본 틀은 1번 프리셋 사용
    itemDataForDetail.item_equipment = Array.from(allItems.values()); // 내용을 합쳐진 전체 목록으로 교체
    ...
    response.status(200).json({
      ...
      // ✨ 프론트엔드가 원래 사용하던 'itemData'라는 이름으로 다시 보내줍니다.
      itemData: itemDataForDetail,
